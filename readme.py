...
st.set_page_config(layout="centered", page_title="The Ultimate Vocabulary Test") # Title updated here for browser tab

# --- Helper Functions (load_words, get_new_word_set, highlight_word_in_sentence, motivational_quotes) ---
@st.cache_data
def load_words():
    ...
    word_data_list = [
        ...
        # Ensure this list is well-populated for the app to function correctly,
        # especially the distractor logic which needs a decent pool of words.
    ]
    if not word_data_list:
        return []
    if len(word_data_list) < 20: # Warn if list is too small for good distractor generation
        st.sidebar.warning(f"Word list is very small ({len(word_data_list)} words). This may affect question variety.")
...
def get_new_word_set(words_list, num_words=10, seen_indices=None):
    ...
    if len(available_indices) < num_words:
        # st.sidebar.warning("Not enough new unique words. Words may repeat.") # Removed to avoid clutter if user doesn't want sidebar
        if not available_indices and words_list:
            seen_indices.clear(); available_indices = list(range(len(words_list)))
...
def setup_next_round_callback():
    # st.sidebar.info("DEBUG (Callback): setup_next_round_callback called.") # Removed sidebar debug
    ...
    st.session_state.current_word_set = get_new_word_set(st.session_state.all_words_loaded, st.session_state.num_words_per_round, st.session_state.seen_words_indices)
...
def restart_game_callback():
    # st.sidebar.info("DEBUG (Callback): restart_game_callback called.") # Removed sidebar debug
    current_quiz_dir = st.session_state.get("quiz_direction", "Polish to English")
    num_words_setting = st.session_state.get("num_words_per_round", 10) # Preserve num_words setting
    loaded_words = st.session_state.get("all_words_loaded", [])
    if not loaded_words and ALL_WORDS: loaded_words = ALL_WORDS
    ...
    st.session_state.num_words_per_round = num_words_setting # Restore
    st.session_state.all_words_loaded = loaded_words
...
default_session_state = {
    ...
    "test_questions_cache": {}, "num_words_per_round": 10, # Default words per round
    "learning_mode": "Timed (5s per word)" # Default learning mode
}
for key, value in default_session_state.items():
    if key not in st.session_state: st.session_state[key] = value
if not st.session_state.all_words_loaded and ALL_WORDS: st.session_state.all_words_loaded = ALL_WORDS

st.markdown("""
    <style>
        /* ... (CSS styles remain the same as previous full code) ... */
        ...
        .welcome-text {font-size: 1.1em; margin-bottom: 15px;}
        .total-words-info {font-style: italic; color: grey; margin-bottom:20px;}
    </style>
""", unsafe_allow_html=True)

st.title("✨ The Ultimate Vocabulary Test ✨") # Title Updated

# --- Main App Logic ---
if st.session_state.stage == "welcome":
    st.header("Welcome to the Vocabulary Trainer!")
    st.markdown("<p class='welcome-text'>Select your preferences and start learning new words!</p>", unsafe_allow_html=True)

    total_words = len(st.session_state.all_words_loaded)
    st.markdown(f"<p class='total-words-info'>Total words currently in vocabulary: {total_words}</p>", unsafe_allow_html=True)
    ...
    st.session_state.num_words_per_round = st.slider(
        "Words per round:", min_value=5, max_value=20, 
        value=st.session_state.get("num_words_per_round", 10), step=1, key="num_words_slider"
    )
    st.session_state.learning_mode = st.radio(
        "Learning Mode:", ("Timed (5s per word)", "Manual (Next Word button)"),
        index=("Timed (5s per word)", "Manual (Next Word button)").index(st.session_state.get("learning_mode", "Timed (5s per word)")),
        key="learning_mode_selector"
    )

    if st.button("🚀 Start Learning"):
        if not st.session_state.all_words_loaded:
            st.error("Word list is empty. Please add words to the `load_words()` function.")
        elif total_words < st.session_state.num_words_per_round and total_words < 5 : # Need enough for one round and distractors
            st.error(f"Not enough words in the vocabulary ({total_words}) to start a round of {st.session_state.num_words_per_round} words or generate test options. Please add more words.")
        else:
            ...
            if not st.session_state.current_word_set:
                st.error("Could not load words for the round. Check vocabulary size."); st.session_state.stage = "welcome"
            else:
    ...
    num_words_in_set = len(st.session_state.current_word_set) if st.session_state.current_word_set else 0
    if not st.session_state.current_word_set: st.error("Word set is empty."); st.session_state.stage = "welcome"; st.rerun()
    elif st.session_state.current_learning_word_index >= num_words_in_set:
        ...
        st.header(f"🧠 Round {st.session_state.round_number}: Learn Word {st.session_state.current_learning_word_index + 1}/{num_words_in_set}")
        
        if st.session_state.learning_mode == "Timed (5s per word)":
        ...
        if st.session_state.learning_mode == "Timed (5s per word)":
            if time_remaining_word <= 0:
                ...
                if st.session_state.current_learning_word_index < num_words_in_set: st.session_state.learning_word_start_time = time.time()
                st.rerun()
            else: time.sleep(1); st.rerun()
        else: # Manual mode
            if st.button("Next Word ➡️", key=f"next_learn_word_{st.session_state.current_learning_word_index}"):
                st.session_state.current_learning_word_index += 1
                if st.session_state.current_learning_word_index < num_words_in_set: st.session_state.learning_word_start_time = time.time() # Reset for potential future use
                st.rerun()


elif st.session_state.stage == "test":
    # ... (Test stage - using the stable option generation logic from the previous full code) ...
    # This stage uses the test_questions_cache correctly.
    quiz_dir = st.session_state.get("quiz_direction", "Polish to English")
    num_words_for_test = len(st.session_state.current_word_set) if st.session_state.current_word_set else 0
    
    # Display progress within the test
    st.subheader(f"Question {st.session_state.get('current_test_question_index', 0) + 1} of {num_words_for_test}")


    test_cache_key = f"test_questions_round_{st.session_state.round_number}_{quiz_dir.replace(' ','_')}"
    if not st.session_state.test_questions_cache.get(test_cache_key):
        # ... (Question generation logic - this is complex and kept from previous working version)
        generated_questions = []
        ...
        if not st.session_state.current_word_set : st.error("Test Error: Word set empty."); st.session_state.stage="welcome"; st.rerun()
        ...
        for i, word_data in enumerate(st.session_state.current_word_set):
            ...
            if distractor_pool:
                num_to_sample = min(4, len(set(distractor_pool)))
                if num_to_sample > 0: chosen_distractors.update(random.sample(list(set(distractor_pool)), num_to_sample))
            ...
            while len(final_options_list) < 5:
                ...
                if padding_candidates: final_options_list.append(random.choice(padding_candidates))
                else: padding = f"Option {idx+random.randint(100,200)}"; final_options_list.append(padding)
                idx +=1;  If idx > 20 : break
            ...
            while len(final_options_list) < 5: final_options_list.append(f"PadOpt{len(final_options_list)}_{random.randint(201,300)}")
            random.shuffle(final_options_list)
            generated_questions.append({
                "question_word": question_word, "correct_answer": correct_answer,
                "options": final_options_list[:5], "form_key": f"q_r{st.session_state.round_number}_{i}_{quiz_dir.replace(' ','_')}_{random.randint(0,10000)}"
            })
        st.session_state.test_questions_cache[test_cache_key] = generated_questions
        st.session_state.current_test_question_index = 0 # Initialize for progress display
    ...
    with st.form(key=f"test_form_r{st.session_state.round_number}_{quiz_dir}_form"):
        temp_answers = {}
        retrieved_questions = st.session_state.test_questions_cache.get(test_cache_key, [])
        if not retrieved_questions and st.session_state.current_word_set : st.error("Error: Questions not loaded."); st.stop()
        
        # For displaying progress X of Y
        if retrieved_questions:
            st.markdown(f"**Progress: Question {st.session_state.get('current_test_question_display_num', 1)} of {len(retrieved_questions)}**")
            # This progress is for display only, the actual questions are all rendered in the form at once.
            # A true "one question at a time" test would require a different structure.

        for i, q_data in enumerate(retrieved_questions):
            # This index 'i' can be used to update a displayed current question number if we switch to one-by-one display
            # st.session_state.current_test_question_index = i # This would change on each form element render, not ideal for "current question"
        ...
        if st.form_submit_button("✅ Submit Answers"):
            ...
            st.rerun()
    if time_remaining_test > 0 and 'submitted_test' not in st.session_state: time.sleep(1); st.rerun()


elif st.session_state.stage == "results":
    # ... (Results stage logic - using callbacks for buttons) ...
    ...
    if not st.session_state.get('test_answers'): st.warning("No answers processed.")
        ...
        num_q = len(st.session_state.current_word_set) if st.session_state.current_word_set else 0
        if num_q > 0: st.subheader(f"You scored {round_score}/{num_q} this round.")
        else: st.subheader("No questions in this round to score.")
    ...
    with col1: st.button("Next Set of Words ➡️", on_click=setup_next_round_callback, use_container_width=True, key=f"next_btn_{st.session_state.round_number}")
    with col2: st.button("Restart Game 🔄", on_click=restart_game_callback, use_container_width=True, key=f"restart_btn_{st.session_state.round_number}")

else:
    st.error("Unknown application stage. Resetting."); st.session_state.stage = "welcome"; st.rerun()
